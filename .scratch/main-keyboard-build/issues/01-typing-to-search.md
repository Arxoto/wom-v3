...
- [x] 输入框受控：值来自交互状态，输入立刻反映在界面上
- [x] 50ms 尾防抖；文本与上次发出的相同就不重发
- [x] 过期响应整包丢弃：先发的请求后回来，不能覆盖新结果
- [x] 拼音合成期间只更新输入框、不检索；`compositionend` 自己补一次检索，与随后那次变更去重后只得到一个结果
- [x] 列表渲染真实结果，可见条数按 `Config` 的 `main_item_n`
- [x] 结果为空时列表区显示一行占位文案
- [x] 输入为空时不检索、结果置空（列表区留白，提示在输入框的 ghost 里）；关键字取第一个空格之前的内容，以空格开头时关键字是空串（照常全量检索）
