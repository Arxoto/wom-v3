...
- [x] 新增无参命令，返回 `ItemType` → 动作列表；顺序即优先级，第一个是默认动作
- [x] 映射表按 spec §4.2 写死；动作带 ASCII 的 `id` 与 `label_key`
- [x] `label_key` 按 `ItemType` + 动作分套（同一个 `copy` 在 `File` 与 `Web` 上文案不同）
- [x] 不按平台剔除动作
- [x] 前端挂载时拉一次动作表；类型镜像补齐
- [x] 每行显示该行默认动作的图标，没有动作时动作栏整块不渲染
- [x] Tail 显示当前 `ItemType` + 动作的中文描述，查不到键就显示键本身
- [x] 本票据不引入切换动作的键位与菜单
