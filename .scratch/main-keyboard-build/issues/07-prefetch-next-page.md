...
- 2026-09-20：补一层 Rust 侧校验——`search_page` 现在要带上「屏上那份结论属于哪个关键字」，后端与缓存比对，对不上直接报错（Rust 记 warn，前端打控制台 + 放开在飞记账）；防抖窗口里已经发出去的那一笔仍由令牌丢弃。见 [spec.md](../main-keyboard/spec.md) §3 / §4.6。
