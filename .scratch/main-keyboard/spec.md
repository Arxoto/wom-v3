...
| `reducer.ts` | `MainState` + 纯函数 `reduce(state, action)`；动作来源只有 `typing` / `settled` / `page_appended` / `intent` / `main_shown` 五类。`settled` 整份换掉 `conclusion`（`page` 为 `null` 表示这一轮没有查询，结论清空），`page_appended` 只在结论后面接一页 |
| `search_session.ts` | 检索会话的时序：50ms 尾防抖（handle 的排 / 取消就地写在这里）、请求令牌、预请求的在飞记账、合成锁（合成期间不排检索，解锁时按读到的当前文本补一次）；搜不搜（空输入不搜、关键字没变不重发）与关键字怎么切（第一个空格之前）都在 `input_changed` 里收口。记账收在一个 `search_state` 里（`last_sent` / `search_token` / `prefetch_in_flight`，令牌与去重记录只有 `next_round` 一处写），**不存阶段**——「这一刻算什么阶段」没有读点，翻页判据直接看外面传进来的结论。上报只有两件事：结论落定（`on_conclusion`，`null` = 这一轮没有查询）与预请求续上了一页——「结果还在路上」不上报，渲染只认最后一份结论。非 React 模块，两个 invoke（返回 Promise）与两个派发回调由接线层注入，判据（已加载条数 / `Selection` / `total`）也从外面传 |
...
| --- | --- |
| `input` | 输入框的值（受控） |
| `conclusion` | 最后一份落定的结论（`item_list` + `total`），分页往后追加；`null` = 还没有结论（没输入过，或输入为空，或这份输入的答复还没回来）。整份替换（只有 `settled` 一处写），所以画面只在结论落定时改变 |
...
- **空结果**：已加载列表为空时，所有依赖条目的意图退化成无操作（判据是列表长度，不是 `selection` 的取值）；列表区显示一行占位文案——没有结论（`conclusion` 为 `null`）留白，输入为空的提示在 `Head` 的 ghost 里，落到确实没匹配才是「没有匹配的条目」；Tail 保持列表模式那套提示（见 [Selection 与 Preview 的行为](issues/06-selection-and-preview.md)）。
...
| 组件 | props |
| --- | --- |
| `Head` | `{ value, ghost, input_ref, read_only, empty, total, selection }`（ref 归 hook，聚焦与全选要用；`ghost` 沿用现状的占位串，本 effort 不实现补全；`empty` 见 §3 的「空态与画面」） |
| `Body` | `{ item_list, selection, item_n, show_preview, type_actions, empty }`（滚动偏移是组件自己的状态；`empty` 见 §3 的「空态与画面」，决定列表为空时那句占位文案） |
...
- **首屏不检索**：挂载时不发那次空关键字，列表空着、输入框上的 ghost 提示输入搜索，直到第一次输入。
- **空输入不检索**：输入变空是**当场落定**的（没有要合并的击键，不必等防抖）：不发请求，直接回调 `on_conclusion(null)`——reducer 把结论清空（`Selection` 与 `Preview` 归零，列表区留白、输入框上的 ghost 回来）。同时在飞的第一页与预请求一并作废，去重记录复位，清空后重打同一个关键字会重新搜。判据是原始输入而不是关键字：只打一个空格时输入非空，照常按空关键字（全量）检索。
- **关键字是第一个空格之前的内容**：空格之后的部分留给条目参数，不参与检索；以空格开头时关键字就是空串，也就是全量。
- **空态与画面**：列表区与右上角标签共用一条判据——**有没有落定的结论**（`MainState.conclusion` 为 `null`），别的状态都不参与。`null` 时两处都留白；有结论时标签是 `N/A`（这一次查询确实 0 条）或「当前选中项的序号 / 结果总数」，列表区只在那份结论确实没匹配时才说「没有匹配的条目」。所以**已经打字、答复还没回来的那一段都保持上一份结论的样子**：打字过程中画面一次都不变，首屏第一次输入不会先说「没有匹配的条目」或闪一个 `N/A`（那会儿还没有结论），上一条结论是 `N/A` 时它也会稳定留着，直到新结论落定才换。`empty` 由 `AppMain` 现算（`conclusion === null`）交给 `Head` / `Body`，两者各只收这一个判据。`Head` 的 ghost 提示不跟它走——它画在已输入的文字后面，只有真没输入时才成立。（2026-09-19 修订：早先「空态 = 输入为空或结果还在路上」与「那 50ms 防抖不在其中」的口径作废，见 [05 号 ticket](issues/05-typing-search-and-ime.md)。）
...
- 每次 `↓` 判断（判据取移动前的 `Selection`，等效余量 9 行）：`selection >= 已加载条数 - 10`（余量写死 10，不跟随 `main_item_n`）且 `已加载条数 < total`，满足就请求下一页。还没有结论时两个数都是 0，后一条先把它挡住——首屏与空结果下按 `↓` 不发请求。
- `search_page` 的 `index` 参数是**结果集里的起始位置**，不是页码；预请求传已加载条数。
- 预请求是纯异步的列表追加：只影响 `item_list`，`Selection` 与滚动位置不因它的成败改变；失败在控制台留痕（Rust 记 warn；预请求的失败自己捕获、`console.error` 之后放开在飞记账，第一页的失败不捕获、由未处理的拒绝自动打印），不向用户提示；只要指针还在后 10 位，下一次 `↓` 会再试。
- 同一时间最多一页在飞（起始下标恒等于已加载条数，而它只在上一页落账之后才增长），所以用单槽记账就够：挡同一页的重复请求，新检索到来时清掉这一笔（同一套令牌规则）。
- 预请求只替屏上那份结论翻页：请求带上它的关键字（见 §4.6），Rust 与缓存比对，对不上就报错、由前端当失败处理。防抖窗口里发出去的那一笔带的是上一轮的令牌，新检索一开步就被丢掉；新检索已发出、结论还没落定的那一小段里，请求走到后端会被拒——不会把新关键字的结果接到旧列表上。
...
### 4.6 `search_page` 带关键字

`search_page({ index, k })`：`k` 是前端**正在展示**的那份结论所属的关键字。新检索一到，后端缓存就换成新关键字的结果集，而前端要等答复回来才换列表——这中间它手里还是旧列表，替旧列表续页会把新关键字的结果接到旧结果后面。后端比对 `k` 与缓存里的 `input_key`，对不上就返回 Err：Rust 记 warn，前端按失败处理（打控制台 + 放开在飞记账），下一次 `↓` 再试。

不返回空页：这不是「没有下一页」，是前端手里那份列表与后端缓存对不上，空页会把这种情况伪装成正常的翻到底。
...
```text
margin   = floor(item_n * 0.4)   // 两侧余量的估算值（行数）
row      = selection - offset    // 高亮落在屏幕的第几行（0 基）

row < margin - 1      → offset = selection - margin + 1        // 踏进上面的余量带：窗口下移
row > item_n - margin → offset = selection + margin - item_n   // 踏进下面的余量带：窗口上移
否则窗口不动

offset = clamp(offset, 0, loaded_len - item_n)  // 列表顶 / 最后一屏
```

效果：10 行时 `margin` 是 4，高亮平时只在屏幕第 4–7 行里自己走；往下按到第 8 行就踏进下面的余量带，窗口整体上移一行、高亮回到第 7 行；往上按到第 3 行就踏进上面的余量带，窗口整体下移一行、高亮回到第 4 行。两条带都够不着时窗口一动不动、高亮自己走——这就是消掉「高亮在线上反复横跳、列表跟着抖」的死区。按 `margin` 数：两侧的余量带各 `margin - 1` 行（10 行时各 3 行），中间的自由区是剩下的行（10 行时 4 行）；高亮踏进哪一侧，就把窗口往那一侧挪一行、把高亮拉回带内的边界行上。

余量要取整到行（行号是拿去跟 `Selection` 比相等的，小数行永远比不中、高亮会丢）：`item_n` 为 1–4 时 `margin` 是 0 或 1，两条带都够不着，整份列表都露着、高亮自己走，也就没有滚动。偏移两端夹住：`offset = 0`（列表顶）与 `offset = loaded_len - item_n`（最后一屏）之后窗口挪不动了，高亮继续走到首行 / 末行，也就是「直至到顶 / 直至到底」。偏移一律不小于 0，所以 `loaded_len` 那一端不用再取一次 `max(0, …)`；已加载不足一屏时右端是负数，被 0 兜住，整份列表都露着。Selection 不动（翻页追加、显示复位）时这一笔整段不发生，偏移也就跟着不动。本 effort 只做最简单的滚动，不含过渡动画。
