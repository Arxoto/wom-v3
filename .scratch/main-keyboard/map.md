...
- [分页与滚动](issues/09-paging-and-scroll.md)：预请求余量写死 10、Selection 落到已加载列表后 10 位就发；高亮停在可见区往下 60% 那一行、列表整体上移，下面没有更多结果才移到末行。（2026-09-19 修订：偏移改成 `Body` 自己的状态，余量按 `margin = floor(item_n * 0.4)` 算，两侧余量带各 `margin - 1` 行，高亮踏进哪一侧才挪窗口，见 `spec.md` §5；失败都在控制台留痕——Rust 记 warn，前端捕获到的自己 `console.error`，见 [预请求下一页](../main-keyboard-build/issues/07-prefetch-next-page.md)。2026-09-20 追加：翻页请求要带上屏上那份结论的关键字，Rust 与缓存比对，对不上报错，见 `spec.md` §4.6。）
