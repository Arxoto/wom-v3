...
- [Selection 与 Preview 的行为](issues/06-selection-and-preview.md)：新结果回到第一条、边界 clamp 不环绕；Preview 跟随 Selection，空结果自动关闭并给一行占位；打开时 input 用 `readOnly`，↑/↓ 与 ←/→ 都不产生意图。
...
- [分页与滚动](issues/09-paging-and-scroll.md)：预请求余量写死 10、Selection 落到已加载列表后 10 位就发（失败静默，Rust 记日志）；高亮停在倒数第二行、列表整体上移，下面没有更多结果才移到末行。
...
- [IME 合成期间的事件语义](issues/01-research-ime-composition.md)：lock（`compositionstart` / `compositionend`，只管 input 路径）；`compositionend` 自己补一次检索，靠「文本没变不重发」去重；不做显示时清 lock 的兜底；keydown 暂不做合成判断，待实测。
