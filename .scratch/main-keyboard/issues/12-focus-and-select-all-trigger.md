...
- 2026-09-19（依代码订正 + 实机验证）：下面那条评论里的 `mousedown` 路径代码里没有，也不需要——`useMainInteraction` 给 input 挂的是 `input` / `compositionstart` / `compositionend` / `focus` 四个监听，实机确认 `focus` 里 `select()` 已经能实现「点一下全选」，按下的默认动作没有把全选冲掉；窗口显示那条路（Rust 发 `main_shown` + 挂载兜底）照旧。
