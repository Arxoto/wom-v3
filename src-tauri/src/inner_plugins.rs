...
impl ItemsStat {
    fn new(items: ItemCollection) -> Self {
...
pub fn reload_settings<R: Runtime>(app: &AppHandle<R>) -> Result<(), Box<dyn std::error::Error>> {
    let items = load_settings(app)?;
...
fn load_settings<R: Runtime>(app: &AppHandle<R>) -> io::Result<ItemCollection> {
...
fn handle_parsed_error(e: ItemParseErr, line_content: &str) {
    match e {
        items::impl_persistence::ItemParseErr::EmptyLine => { /* empty */ }
        items::impl_persistence::ItemParseErr::ValueNotEnough(err_msg) => {
            log::warn!("load inner db error with: {}", err_msg);
        }
        items::impl_persistence::ItemParseErr::ItemTypeParsedFailed => {
            log::warn!("load inner db error by unknown type");
            log::debug!("unknown type by line: {}", line_content);
        }
        items::impl_persistence::ItemParseErr::ItemValueParsedFailed(err_msg) => {
