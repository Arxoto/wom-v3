...
落地：`src/main/interaction/`（`keys.ts` / `reducer.ts` / `search_session.ts` / `useMainInteraction.ts`），检索的时序（防抖、请求令牌、预请求记账、合成锁）收在 `search_session.ts`。退场、失焦隐藏、触发动作后是否隐藏、显示时聚焦并全选，由 Rust 侧按 `Config` 与窗口事件决定，前端只表达意图。
